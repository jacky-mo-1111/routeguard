...
(combined / train_all / policyguard), then write a single result.txt that lets us
...
Inputs (assumed layout under --root):
    ...
    <root>/combined/policyguard_guardrail_eval/generated_predictions.jsonl
    <root>/train_all/test_eval/generated_predictions.jsonl
    <root>/policyguard/policyguard_guardrail_eval/generated_predictions.jsonl
...
# policyguard_guardrail_{train,eval}: violation / no_violation (order: no_violation before violation
# to avoid matching the substring "violation" inside "no_violation")
POLICY_RE = re.compile(r"(no_violation|violation)\b", re.IGNORECASE)


def extract_label(text: str) -> Optional[str]:
    """Map to a binary safe/unsafe view: test_eval uses Qwen guard; policyguard uses violation ≈ unsafe."""
    if not text:
        return None
    t = str(text).strip()
...
def fmt_row(name: str, s: Stats, *, policy_violation: bool = False) -> str:
    f1_a, f1_b = (
        ("f1_violation", "f1_no_violation")
        if policy_violation
        else ("f1_unsafe", "f1_safe")
    )
    return (
...
def main() -> None:
    ...
    ap.add_argument("--root", required=True, help="results root, e.g. results/qwen3_06b_policyguard")
    args = ap.parse_args()
    ...
    plan = [
        (
            "combined",
            "[1] Combined  (train: train_all + policyguard_guardrail_train)",
            ["test_eval", "policyguard_guardrail_eval"],
        ),
        (
            "train_all",
        ...
        (
            "policyguard",
            "[2.2] Separate (train: policyguard_guardrail_train only)",
    ...
    lines: list[str] = []
    lines.append("=" * 88)
    lines.append("qwen3_guard_gen_0_6b -- combined-vs-separate training, policyguard experiment")
    lines.append("=" * 88)
    for cfg, header, splits in plan:
        ...
        for sp, st in res.items():
            lines.append(fmt_row(sp, st, policy_violation=(sp == "policyguard_guardrail_eval")))
    ...
    def _delta(
        ...
        *,
        ...
        if c is None or s is None:
            ...
            return out
        out.append(fmt_row("combined", c, policy_violation=policy_violation))
        out.append(fmt_row("separate", s, policy_violation=policy_violation))
        a, b = (
            ("f1_violation", "f1_no_violation")
            if policy_violation
            else ("f1_unsafe", "f1_safe")
        ...
        out.append(
            ...
            f"{a}={(c.f1_unsafe - s.f1_unsafe) * 100:+6.2f}%  "
            f"{b}={(c.f1_safe - s.f1_safe) * 100:+6.2f}%"
        )
        return out
    ...
    lines.append("")
    lines.extend(_delta(
        "On policyguard_guardrail_eval:",
        all_results.get("combined", {}).get("policyguard_guardrail_eval"),
        all_results.get("policyguard", {}).get("policyguard_guardrail_eval"),
