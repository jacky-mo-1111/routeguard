...
#   [1]   combined : train=train_all + policyguard_guardrail_train , eval=test_eval + policyguard_guardrail_eval
#   [2.1] separate : train=train_all                               , eval=test_eval
#   [2.2] separate : train=policyguard_guardrail_train             , eval=policyguard_guardrail_eval
#
...
#
# Usage:
#   bash examples/qwen3_06b_policyguard_pipeline/run_pipeline.sh
...
PIPE="examples/qwen3_06b_policyguard_pipeline"
RESULT_ROOT="/nas02/jacky/Debug_LM/results/qwen3_06b_policyguard"
...
if [[ "${ONLY_METRICS:-0}" != "1" ]]; then
  if [[ "${SKIP_TRAIN:-0}" != "1" ]]; then
    ...
    run "TRAIN [1] combined  (train_all + policyguard_guardrail_train)" \
    ...
    run "TRAIN [2.2] policyguard_guardrail_train only" \
      llamafactory-cli train "${PIPE}/train_policyguard.yaml"
  else
    banner "SKIP_TRAIN=1: skipping all 3 training stages"
  fi

  if [[ "${SKIP_EVAL:-0}" != "1" ]]; then
    run "EVAL  [1] combined ckpt  → test_eval + policyguard_guardrail_eval" \
    ...
    run "EVAL  [2.2] policyguard ckpt → policyguard_guardrail_eval" \
      llamafactory-cli train "${PIPE}/eval_policyguard.yaml"
  else
