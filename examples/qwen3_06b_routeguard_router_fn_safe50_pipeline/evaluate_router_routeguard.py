#!/usr/bin/env python3
"""Evaluate RouteGuard router v2 with precomputed local expert predictions.
...
def main() -> None:
    ...
    ap.add_argument("--router-pred", type=Path, default=Path("/nas02/jacky/Debug_LM/results/qwen3_06b_routeguard_router_fn_safe50/router/generated_predictions.jsonl"))
    ...
    ap.add_argument("--out-dir", type=Path, default=Path("/nas02/jacky/Debug_LM/results/qwen3_06b_routeguard_router_fn_safe50"))
    args = ap.parse_args()
    ...
    gold_route_support: Counter[str] = Counter()
    gold_route_hit: Counter[str] = Counter()
    router_safe_on_unsafe = 0
    router_unsafe_on_safe = 0
    gold_empty_braces = 0
    ...
    for i, row in enumerate(router_rows):
        ...
        gold_routes = {expert for expert in EXPERT_ORDER if gold & EXPERT_CATS[expert]}
        if gold and not routes:
            router_safe_on_unsafe += 1
        if (not gold) and routes:
            router_unsafe_on_safe += 1
        for expert in EXPERT_ORDER:
            if expert in gold_routes:
                gold_route_support[expert] += 1
                if expert in routes:
                    gold_route_hit[expert] += 1
    ...
    lines: list[str] = []
    lines.append("Baseline vs RouteGuard Router FN-Safe50")
    lines.append("=" * 112)
    ...
    lines.append("")
    lines.append("Router route recall against gold expert routes:")
    lines.append(f"  gold unsafe but router predicted SAFE: {router_safe_on_unsafe}")
    lines.append(f"  gold safe but router predicted expert(s): {router_unsafe_on_safe}")
    for expert in EXPERT_ORDER:
        support = gold_route_support[expert]
        hit = gold_route_hit[expert]
        recall = hit / support if support else 0.0
        lines.append(f"  {expert:<11s} hit={hit} support={support} recall={pct(recall)}")
    lines.append("")
    ...
    result = {
        ...
        "gold_route_support": dict(gold_route_support),
        "gold_route_hit": dict(gold_route_hit),
        "router_safe_on_unsafe": router_safe_on_unsafe,
        "router_unsafe_on_safe": router_unsafe_on_safe,
