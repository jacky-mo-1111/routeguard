...
(combined / train_all / attackactor), then write a single result.txt that lets us
...
Inputs (assumed layout under --root):
    ...
    <root>/combined/actorattack_guardrail_eval/generated_predictions.jsonl
    <root>/train_all/test_eval/generated_predictions.jsonl
    <root>/attackactor/actorattack_guardrail_eval/generated_predictions.jsonl
...
def main() -> None:
    ...
    ap.add_argument("--root", required=True, help="results root, e.g. results/qwen3_06b_attackactor")
    args = ap.parse_args()
    ...
    plan = [
        (
            "combined",
            "[1] Combined  (train: train_all + actorattack_guardrail_train)",
            ["test_eval", "actorattack_guardrail_eval"],
        ),
        (
            "train_all",
        ...
        (
            "attackactor",
            "[2.2] Separate (train: actorattack_guardrail_train only)",
    ...
    lines: list[str] = []
    lines.append("=" * 88)
    lines.append("qwen3_guard_gen_0_6b -- combined-vs-separate training, attackactor experiment")
    lines.append("=" * 88)
    ...
    lines.append("")
    lines.extend(_delta(
        "On actorattack_guardrail_eval:",
        all_results.get("combined", {}).get("actorattack_guardrail_eval"),
        all_results.get("attackactor", {}).get("actorattack_guardrail_eval"),
    ))
