...
#   [1]   combined : train=train_all + redcoder_guardrail_train , eval=test_eval + redcoder_guardrail_eval
#   [2.1] separate : train=train_all                            , eval=test_eval
#   [2.2] separate : train=redcoder_guardrail_train            , eval=redcoder_guardrail_eval
#
...
#
# Usage:
#   bash examples/qwen3_06b_redcoder_pipeline/run_pipeline.sh
...
PIPE="examples/qwen3_06b_redcoder_pipeline"
RESULT_ROOT="/nas02/jacky/Debug_LM/results/qwen3_06b_redcoder"
...
if [[ "${ONLY_METRICS:-0}" != "1" ]]; then
  if [[ "${SKIP_TRAIN:-0}" != "1" ]]; then
    ...
    run "TRAIN [1] combined  (train_all + redcoder_guardrail_train)" \
    ...
    run "TRAIN [2.2] redcoder_guardrail_train only" \
      llamafactory-cli train "${PIPE}/train_redcoder.yaml"
  else
    banner "SKIP_TRAIN=1: skipping all 3 training stages"
  fi

  if [[ "${SKIP_EVAL:-0}" != "1" ]]; then
    run "EVAL  [1] combined ckpt  → test_eval + redcoder_guardrail_eval" \
    ...
    run "EVAL  [2.2] redcoder ckpt → redcoder_guardrail_eval" \
      llamafactory-cli train "${PIPE}/eval_redcoder.yaml"
  else
