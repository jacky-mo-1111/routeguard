...
(combined / train_all / or_bench), then write a single result.txt that lets us
...
Inputs (assumed layout under --root):
    ...
    <root>/combined/or_bench_ob_eval/generated_predictions.jsonl
    <root>/train_all/test_eval/generated_predictions.jsonl
    <root>/or_bench/or_bench_ob_eval/generated_predictions.jsonl
...
def main() -> None:
    ...
    ap.add_argument("--root", required=True, help="results root, e.g. results/qwen3_06b_orbench")
    args = ap.parse_args()
    ...
    plan = [
        (
            "combined",
            "[1] Combined  (train: train_all + or_bench_ob_train)",
            ["test_eval", "or_bench_ob_eval"],
        ),
        (
            "train_all",
        ...
        (
            "or_bench",
            "[2.2] Separate (train: or_bench_ob_train only)",
            ["or_bench_ob_eval"],
        ),
    ]
    ...
    lines: list[str] = []
    lines.append("=" * 88)
    lines.append("qwen3_guard_gen_0_6b — combined-vs-separate training, or_bench experiment")
    lines.append("=" * 88)
    ...
    lines.append("")
    lines.append("=" * 88)
    lines.append("Comparison: combined vs separate (Δ = combined − separate)")
    lines.append("=" * 88)
    ...
    def _delta(label: str, c: Optional[Stats], s: Optional[Stats]) -> list[str]:
        ...
        out.append(
            f"  {'Δ':<22s} acc={(c.acc - s.acc) * 100:+6.2f}%  "
    ...
    lines.extend(_delta(
        "On or_bench_ob_eval:",
        all_results.get("combined", {}).get("or_bench_ob_eval"),
        all_results.get("or_bench", {}).get("or_bench_ob_eval"),
    ))
