...
#   [1]   combined : train=train_all + or_bench_ob_train , eval=test_eval + or_bench_ob_eval
#   [2.1] separate : train=train_all                     , eval=test_eval
#   [2.2] separate : train=or_bench_ob_train             , eval=or_bench_ob_eval
#
...
#
# Usage:
#   bash examples/qwen3_06b_orbench_pipeline/run_pipeline.sh
...
PIPE="examples/qwen3_06b_orbench_pipeline"
RESULT_ROOT="/nas02/jacky/Debug_LM/results/qwen3_06b_orbench"
...
if [[ "${ONLY_METRICS:-0}" != "1" ]]; then
  if [[ "${SKIP_TRAIN:-0}" != "1" ]]; then
    ...
    run "TRAIN [1] combined  (train_all + or_bench_ob_train)" \
    ...
    run "TRAIN [2.2] or_bench_ob_train only" \
      llamafactory-cli train "${PIPE}/train_or_bench.yaml"
  else
    banner "SKIP_TRAIN=1: skipping all 3 training stages"
  fi

  if [[ "${SKIP_EVAL:-0}" != "1" ]]; then
    run "EVAL  [1] combined ckpt  → test_eval + or_bench_ob_eval" \
    ...
    run "EVAL  [2.2] or_bench  ckpt → or_bench_ob_eval" \
      llamafactory-cli train "${PIPE}/eval_or_bench.yaml"
  else
